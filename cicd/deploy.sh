...
# render by env first, then render by json, finally apply to k8s cluster

# Check if $ENV is production, staging, dev or qa
if [[ "$ENV" == "production" ]] || [[ "$ENV" == "staging" ]] || [[ "$ENV" == "dev" ]]; then
  APP_CONSUL_PATH="$ENV_CONSUL_PREFIX/degate/frontend/$SERVICE_NAME/latest/"
else
  APP_CONSUL_PATH="QA/degate/frontend/$SERVICE_NAME/$ENV_CONSUL_PREFIX/"
fi
export APP_CONSUL_PATH
