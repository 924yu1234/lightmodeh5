...
export default function SwapHots({ height }: { height: number }) {
  const intl = useIntl();
  const { orderBy = '', orderDir = '' } = useSort('swap_hots');
  const pageSize = 1000;
  const current = 1;

  const { list: pairs, total } = useSwapPairs({
    current,
    pageSize,
  ...
  const updateSort = useUpdateSort('swap_hots');
