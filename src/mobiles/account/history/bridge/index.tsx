...
import { useFetchBridgeUsdcHistory } from 'src/components/BridgeUsdc/service';
...
export default function MBridgeHistory({ height }: { height: number }) {
  const pageSize = 20;
  const dexAccount = useDexAccount();
  const fetchBridgeUsdcHistory = useFetchBridgeUsdcHistory();
  ...
  const loadMore = useCallback(
    (page: number, end: number, isRefresh = false) => {
      return fetchBridgeUsdcHistory({
        current: page,
        pageSize,
        end,
      })
    ...
    [fetchBridgeUsdcHistory, intl, pageSize]
  );
  ...
  const hasUnsettledFirstPage = useMemo(() => {
    ...
    return firstPage.some(
      (item: any) => item.status === IntentOrderStatus.processing
    );
  }, [data, pageSize]);
  ...
  return (
    <StyledHistory className="m-list bridge" id="mobileBridgeHistory">
      <InfiniteList
        ...
        height={height}
        scrollableTarget="mobileBridgeHistory"
      >
        {loading &&
          new Array(6)
            .fill(0)
