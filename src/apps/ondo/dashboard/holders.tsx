...
import { DailyHoldersApiItem } from './service';
...
function buildChartData(apiData: DailyHoldersApiItem[]): {
  ...
  for (const item of apiData) {
    if (!item.symbol || typeof item.holders !== 'number') {
      // 跳过无效数据
      // eslint-disable-next-line no-continue
      continue;
    }

    const day = dayjs(item.day).format('YYYY-MM-DD');
...
export default function Holders({
  ...
  const { rows, symbols } = useMemo(() => {
    ...
    return {
      ...chartData,
      symbols: sortedSymbols.slice(0, 20), // 只取前20个
    };
  }, [holdersData]);
  ...
  return (
    ...
    >
      <div className="card-title">
        {intl.stocks.Holders_per_Stock_top_20}
        {!showFullScreen && (
