...
export default function Kline({ poolAddress }: { poolAddress: string }) {
  ...
  return (
    <StyledChart className="chart-container">
      {!baseToken ? (
        <div className="chart-top skeleton">
          <Skeleton
            height={viewWidth >= 2560 ? 36 : 32}
            width={viewWidth >= 2560 ? 36 : 32}
            circle
          />
          <Skeleton height={18} width={100} style={{ marginLeft: 4 }} />
        </div>
      ) : (
        <div className="chart-top">
          <TokenIcon
            token={baseToken}
            size={viewWidth >= 2560 ? 36 : 32}
            hideChainIcon
          />
