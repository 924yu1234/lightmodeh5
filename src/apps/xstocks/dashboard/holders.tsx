...
import { DailyHoldersApiItem, fetchDailyHolders } from './service';
...
function buildChartData(apiData: DailyHoldersApiItem[]): {
  rows: ChartRow[];
  symbols: string[];
} {
  ...
  for (const item of apiData) {
    if (!item.symbol || typeof item.cumulative_holders !== 'number') {
      // 跳过无效数据
    ...
    row[item.symbol] = item.cumulative_holders;
  }
  ...
  const rows = sortedRows.map((row, index) => {
    ...
    for (const symbol of symbols) {
      const current = filledRow[symbol] as number | undefined;
      if (typeof current !== 'number') {
        const previousRow = index > 0 ? sortedRows[index - 1] : undefined;
        const prev =
          previousRow && typeof previousRow[symbol] === 'number'
            ? (previousRow[symbol] as number)
            : 0;
        filledRow[symbol] = prev;
      }
    }
    return filledRow;
  });
...
// 优化的自定义 Tooltip 组件 - 一行显示2个token数据
...
export default function Holders({
  ...
  const { data = [] } = useQuery<DailyHoldersApiItem[]>({
    queryKey: ['xstocks', 'holdersData'],
    queryFn: () => fetchDailyHolders(),
  });

  const holdersData = useMemo(() => {
    if (!data || !codesString) return [];
    const codes = codesString.split(',');
    return data.filter((item) => codes.includes(item.address));
  }, [data, codesString]);
  ...
  const { rows, symbols } = useMemo(() => {
    ...
    const chartData = buildChartData(holdersData);
    // 按最后一期数据排序 symbols
    const latestRow = chartData.rows[chartData.rows.length - 1];
    const sortedSymbols = chartData.symbols.sort((a, b) => {
      if (!latestRow) return a.localeCompare(b);
      const aValue = (latestRow[a] as number) || 0;
      const bValue = (latestRow[b] as number) || 0;
  ...
  if (!rows.length || !symbols.length) {
    return (
      <Card ref={ref} style={{ width, height }}>
        <div className="card-title">{intl.stocks.Holders_per_Stock}</div>
        <div
          className="chart"
