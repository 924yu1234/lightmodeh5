...
function buildChartData(apiData: DailyTransactionVolumeApiItem[]): {
  ...
  for (const item of apiData) {
    ...
    const currentValue = Number(row[symbol]) || 0;
    const newValue = Number(item.volume_usd) || 0;
    row[symbol] = currentValue + newValue;
  }
  ...
  const rows = Array.from(byDate.values())
    ...
    .map((row) => {
      ...
      for (const symbol of symbols) {
        if (typeof filledRow[symbol] !== 'number') {
          filledRow[symbol] = 0;
        }
      }
      return filledRow;
    ...
    return (
      <Card ref={ref} style={{ width, height }}>
        <div className="card-title">
          {intl.stocks.Trading_volume_DEX_per_Stock}
        </div>
        <div
          className="chart"
