...
export default function IncreaseInvestmentView({
  order,
}: {
  ...
  const intl = useIntl();
  const params =
    order.increaseInvestmentOrderParams as TurboRangeIncreaseInvestmentOrderParams;

  return (
    <StyledOrder>
      <div className="item">
        <div className="item-title">{intl.amount}</div>
        <div className="item-desc">{params?.amount} USDC</div>
      </div>
      <div className="item">
        <div className="item-title">{intl.est_network_fee}</div>
        <div className="item-desc gas-fee">
          <EstNetworkFeeInConfirm tryResp={params?.tryResp} />
        </div>
      </div>
    </StyledOrder>
  );
