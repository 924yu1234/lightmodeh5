...
import UsdcSuppliedInDetailModal from 'src/components/UsdcSupplied/inDetail';
...
export default function DepositOrderDetailModal() {
  const { visible, hide, order } = useModals(ModalKeys.earnDepositDetail);
    ...
    <BottomModal onClose={hide} opened={visible}>
      <StyledDetail className="modal-wrapper">
        ...
        <div className="modal-content hideScrollBar">
          <TokenIcon token={data.token} hideChainIcon={data.isUsdc} />
          <div className="amount">
            {data.amount_display} {data.token?.symbol}
          </div>
          <UsdcSuppliedInDetailModal tokens={data.usdc_tokens || []} />
