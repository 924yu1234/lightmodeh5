...
const AF_URL_SCHEME = '(https:\\/\\/)(([^\\.]+).)(.*\\/)(.*)';
const VALID_AF_URL_PARTS_LENGTH = 5;
const GOOGLE_CLICK_ID = 'gclid';
const FACEBOOK_CLICK_ID = 'fbclid';
const TWITTER_CLICK_ID = 'twclid';
const SNAPCHAT_CLICK_ID = 'sccid';
const TIKTOK_CLICK_ID = 'ttclid';
const GBRAID = 'gbraid';
const WBRAID = 'wbraid';
const ASSOCIATED_AD_KEYWORD = 'keyword';
const AF_KEYWORDS = 'af_keywords';
const AF_CUSTOM_EXCLUDE_PARAMS_KEYS = [
  'pid',
  'c',
  'af_channel',
  'af_ad',
...
const GCLID_EXCLUDE_PARAMS_KEYS = [
  'pid',
  'c',
  'af_channel',
  'af_ad',
...
const isSkippedURL = function isSkippedURL(_ref) {
  const url = _ref.url;
  const skipKeys = _ref.skipKeys;
  const errorMsg = _ref.errorMsg;
  // search if this page referred and contains one of the given keys
  if (url) {
    const lowerURL = url.toLowerCase();
    if (lowerURL) {
      const skipKey = skipKeys.find(function (key) {
...
const getGoogleClickIdParameters = function getGoogleClickIdParameters(
  gciKey,
  currentURLParams
) {
...
const stringifyParameters = function stringifyParameters() {
  const parameters =
    arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
  const paramStr = Object.keys(parameters).reduce(function (curr, key) {
    if (parameters[key]) {
...
const getParameterValue = function getParameterValue(currentURLParams) {
  const config =
  ...
  const _config$keys = config.keys;
  const keys = _config$keys === void 0 ? [] : _config$keys;
  const _config$overrideValue = config.overrideValues;
  const overrideValues =
    _config$overrideValue === void 0 ? {} : _config$overrideValue;
  const _config$defaultValue = config.defaultValue;
  const defaultValue =
    _config$defaultValue === void 0 ? '' : _config$defaultValue;
  const firstMatchedKey = keys.find(function (key) {
...
const isIOS = function isIOS(useragent) {
  return /iphone|ipad|ipod/i.test(useragent && useragent.toLowerCase());
};
const isUACHSupported = function isUACHSupported() {
  return (
...
const getQueryParamsAndSaveToLocalStorage =
  function getQueryParamsAndSaveToLocalStorage(websiteUrl) {
    ...
    try {
      ...
      const queryParams = Array.from(urlParams).reduce(function (acc, _ref2) {
      ...
      const savedQueryParams = JSON.parse(
      ...
      const dataToSave = _objectSpread2(
...
const isValidUrl = function isValidUrl(urlString) {
  try {
...
const getCurrentUrl = function getCurrentUrl() {
  return new URL(window.location.href);
};
const getReferrerUrl = function getReferrerUrl() {
  const referrer = document.referrer;
  return referrer ? new URL(referrer) : null;
};
const isSameOrigin = function isSameOrigin(currentUrl, refUrl) {
  return currentUrl.origin === refUrl.origin;
};
const saveWebReferrer = function saveWebReferrer() {
  const currentUrl = getCurrentUrl();
  const refUrl = getReferrerUrl();
...
const removeExpiredLocalStorageItems =
  function removeExpiredLocalStorageItems() {
    const currentTime =
      arguments.length > 0 && arguments[0] !== undefined
        ? arguments[0]
        : Date.now();
    const incomingParams = JSON.parse(
      localStorage.getItem('ss_incoming_params') || '[]'
    );
    localStorage.setItem(
      'ss_incoming_params',
      JSON.stringify(
        incomingParams.filter(function (_ref4) {
          const af_ss_exp_at = _ref4.af_ss_exp_at;
...
function getCurrentURLParams(aggregateValues) {
  ...
  if (Object.keys(localStorage).includes('ss_incoming_params')) {
    const incomingParamsFromSS = JSON.parse(localStorage.ss_incoming_params);
    currentURLParams =
...
const isOneLinkURLValid = function isOneLinkURLValid(oneLinkURL) {
  let _ref5;
  const oneLinkURLParts =
...
const isMSValid = function isMSValid() {
  const mediaSource =
    arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
  if (
    !(
...
const isSkipListsValid = function isSkipListsValid(_ref6) {
  const _ref6$referrerSkipLis = _ref6.referrerSkipList;
  const referrerSkipList =
    _ref6$referrerSkipLis === void 0 ? [] : _ref6$referrerSkipLis;
  const _ref6$urlSkipList = _ref6.urlSkipList;
  const urlSkipList = _ref6$urlSkipList === void 0 ? [] : _ref6$urlSkipList;
  if (
    isSkippedURL({
...
const extractCustomParams = function extractCustomParams(_ref7) {
  const _ref7$afCustom = _ref7.afCustom;
  const afCustom = _ref7$afCustom === void 0 ? [] : _ref7$afCustom;
  const _ref7$currentURLParam = _ref7.currentURLParams;
  const currentURLParams =
    _ref7$currentURLParam === void 0 ? {} : _ref7$currentURLParam;
  const googleClickIdKey = _ref7.googleClickIdKey;
  const afParams = {};
  if (Array.isArray(afCustom)) {
    afCustom.forEach(function (customParam) {
      if (
        ...
        const isOverrideExistingKey = AF_CUSTOM_EXCLUDE_PARAMS_KEYS.find(
          function (k) {
            return (
              k ===
...
const validateAndMappedParams = function validateAndMappedParams() {
  const params =
    arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
  const currentURLParams =
  ...
  const ad = params.ad;
  const adSet = params.adSet;
  const deepLinkValue = params.deepLinkValue;
  const afSub1 = params.afSub1;
  const afSub2 = params.afSub2;
  const afSub3 = params.afSub3;
  const afSub4 = params.afSub4;
  const afSub5 = params.afSub5;
  const afCustom = params.afCustom;
  const googleClickIdKey = params.googleClickIdKey;
  const afParams = {};
  ...
  if (mediaSource) {
    ...
    const pidParamKey = isDirectClick ? 'af_media_source' : 'pid';
    afParams[pidParamKey] = pidValue;
  }
  if (campaign) {
    const campaignValue = getParameterValue(currentURLParams, campaign);
  ...
  if (googleClickIdKey) {
    if (
      ...
      const googleParameters = getGoogleClickIdParameters(
        googleClickIdKey,
        currentURLParams
      );
...
const isPlatformValid = function isPlatformValid(platform) {
  if (!platform) {
...
const createImpressionsLink = function createImpressionsLink(finalURL) {
  if (!finalURL) {
...
function updateFinalUrlWithForwardParameters(
  ...
  return forwardParametersList.reduce(function (updatedUrl, parameterName) {
    const parameterValue = getParameterValueFromURL(
      URLSearchParams,
      parameterName
    );
    if (parameterValue) {
...
const processTrackingParameters = function processTrackingParameters(
  destinationURL
) {
  const forwardParametersList = [
    GOOGLE_CLICK_ID,
    FACEBOOK_CLICK_ID,
    TWITTER_CLICK_ID,
    SNAPCHAT_CLICK_ID,
  ...
  const hasGclidOnURL = getParameterValueFromURL(
    window.location.search,
    GOOGLE_CLICK_ID
  );
  const gbraid = getParameterValueFromURL(window.location.search, GBRAID);
  const wbraid = getParameterValueFromURL(window.location.search, WBRAID);
  if (hasGclidOnURL || gbraid || wbraid) {
    const keywordParam = getParameterValueFromURL(
      window.location.search,
      ASSOCIATED_AD_KEYWORD
    );
    const hasAfKeywordOnFinalURL = getParameterValueFromURL(
      destinationURL,
      AF_KEYWORDS
    );
...
function QRCode() {
  ...
  const Drawing = !_isSupportCanvas()
    ? (function () {
        const Drawing = function Drawing(el, htOption) {
          this._el = el;
        ...
        Drawing.prototype.draw = function (oQRCode) {
          ...
          const nonRequiredColorDark = _htOption.colorDark;
          const nonRequiredcolorLight = _htOption.colorLight;
          if (_htOption.backgroundImage) {
            ...
            const backgroundImageEle = `<div style="display:inline-block; z-index:-10;position:absolute;"><img src="${
              _htOption.backgroundImage
            }" width="${_htOption.width + _htOption.quietZone * 2}" height="${
          ...
          if (_htOption.quietZone) {
            divStyle = `display:inline-block; width:${
              ...
              _htOption.width + _htOption.quietZone * 2
            }px;background:${_htOption.quietZoneColor}; text-align:center;`;
          }
          aHTML.push(`<div style="font-size:0;${divStyle}">`);
          aHTML.push(
            `<table  style="font-size:0;border:0;border-collapse:collapse; margin-top:${_htOption.quietZone}px;" border="0" cellspacing="0" cellspadding="0" align="center" valign="middle">`
          );
          aHTML.push(
            `<tr height="${_htOption.titleHeight}" align="center"><td style="border:0;border-collapse:collapse;margin:0;padding:0" colspan="${nCount}">`
          );
          if (_htOption.title) {
            const c = _htOption.titleColor;
            const f = _htOption.titleFont;
            aHTML.push(
              `<div style="width:100%;margin-top:${_htOption.titleTop}px;color:${c};font:${f};background:${_htOption.titleBackgroundColor}">${_htOption.title}</div>`
            );
          }
          if (_htOption.subTitle) {
            aHTML.push(
              ...
              }px;color:${_htOption.subTitleColor}; font:${
          ...
          for (let row = 0; row < nCount; row++) {
            ...
            for (let col = 0; col < nCount; col++) {
              ...
              if (eye) {
                ...
                aHTML.push(
                  `<td style="border:0;border-collapse:collapse;padding:0;margin:0;width:${nWidth}px;height:${nHeight}px;">` +
                    `<span style="width:${nWidth}px;height:${nHeight}px;background-color:${
                ...
                if (row == 6) {
                  ...
                  aHTML.push(
                    `<td style="border:0;border-collapse:collapse;padding:0;margin:0;width:${nWidth}px;height:${nHeight}px;">` +
                      `<div style="display:inline-block;width:${drawWidth}px;height:${drawHeight}px;background-color:${
          ...
          if (_htOption.logo) {
            ...
            let imgDivStyle = `position:relative; z-index:1;display:table-cell;top:-${
              (_htOption.height - _htOption.titleHeight) / 2 +
              imgH / 2 +
              _htOption.quietZone
            }px;text-align:center; width:${imgW}px; height:${imgH}px;line-height:${imgW}px; vertical-align: middle;`;
            if (!_htOption.logoBackgroundTransparent) {
              imgDivStyle += `background:${_htOption.logoBackgroundColor}`;
            }
            aHTML.push(
              `<div style="${imgDivStyle}"><img  src="${_htOption.logo}"  style="max-width: ${imgW}px; max-height: ${imgH}px;" /> <div style=" display: none; width:1px;margin-left: -1px;"></div></div>`
            );
          }
    ...
    : (function () {
        ...
        function _onMakeImage() {
          if (this._htOption.drawer == 'svg') {
            const svgData = this._oContext.getSerializedSvg(true);
        ...
        function _safeSetDataURI(fSuccess, fFail) {
          ...
          if (self._bSupportDataURI === null) {
            ...
            const fOnError = function fOnError() {
              self._bSupportDataURI = false;
              if (self._fFail) {
                self._fFail.call(self);
              }
            };
            const fOnSuccess = function fOnSuccess() {
        ...
        Drawing.prototype.draw = function (oQRCode) {
          ...
          if (_htOption.backgroundImage) {
            ...
            bgImg.onload = function () {
              ...
              const imageSmoothingQuality = _oContext.imageSmoothingQuality;
              const imageSmoothingEnabled = _oContext.imageSmoothingEnabled;
          ...
          function drawQrcode(oQRCode) {
            ...
            for (let row = 0; row < nCount; row++) {
              for (let col = 0; col < nCount; col++) {
                const nLeft = col * nWidth + _htOption.quietZone;
                const nTop = row * nHeight + _htOption.quietZone;
                let bIsDark = oQRCode.isDark(row, col);
                const eye = oQRCode.getEye(row, col); // { isDark: true/false, type: PO_TL, PI_TL, PO_TR, PI_TR, PO_BL, PI_BL };
            ...
            function generateLogoImg(img) {
              let imgContainerW = Math.round(_htOption.width / 3.5);
              ...
              let imgContainerY =
                (_htOption.height +
                  ...
                  imgContainerH) /
                2;
              const imgScale = Math.min(imgContainerW / nw, imgContainerH / nh);
              const imgW = nw * imgScale;
              const imgH = nh * imgScale;
              if (_htOption.logoMaxWidth || _htOption.logoMaxHeight) {
                imgContainerW = imgW;
                imgContainerH = imgH;
                imgContainerX =
...
const formatVersion = version.replace(/\./g, '_'); // replace . with _

QRCode();

// CRITICAL FIX: Ensure QRCode is available on window object
// The QRCode() function above exports QRCode differently based on environment:
// - In CommonJS/Node.js: exports to module.exports
// - In browser global: exports to window.QRCode
// When this file is imported as an ES module in a bundler (Webpack/Vite),
// the module/exports objects exist, so QRCode gets exported as a module
// instead of being set on window. We need to explicitly set it on window
// for the displayQrCode function to work.
if (typeof window !== 'undefined') {
  // Force check: If window.QRCode is not a function, something went wrong
  if (typeof window.QRCode !== 'function') {
    console.warn(
      '[OneLink] QRCode not found on window, attempting to restore from module system'
    );

    // Try to get QRCode from module exports (webpack/vite may have captured it)
    if (typeof module !== 'undefined' && module.exports) {
      if (typeof module.exports === 'function') {
        // QRCode is the default export
        window.QRCode = module.exports;
      } else if (module.exports.QRCode) {
        // QRCode is a named export
        window.QRCode = module.exports.QRCode;
      }
    }

    // Final validation
    if (typeof window.QRCode === 'function') {
      console.debug('[OneLink] QRCode restored to window object successfully');
    } else {
      console.error(
        '[OneLink] Failed to set QRCode on window object - QR code generation will not work'
      );
    }
  } else {
    console.debug('[OneLink] QRCode already available on window object');
  }
}
...
saveWebReferrer();
(function () {
  const generateOneLinkURL = function generateOneLinkURL() {
    const parameters =
      arguments.length > 0 && arguments[0] !== undefined
        ? arguments[0]
        : {
    ...
    const oneLinkURL = parameters.oneLinkURL;
    const _parameters$afParamet = parameters.afParameters;
    const _parameters$afParamet2 =
      _parameters$afParamet === void 0 ? {} : _parameters$afParamet;
    const mediaSource = _parameters$afParamet2.mediaSource;
    const _parameters$referrerS = parameters.referrerSkipList;
    const referrerSkipList =
      _parameters$referrerS === void 0 ? [] : _parameters$referrerS;
    const _parameters$urlSkipLi = parameters.urlSkipList;
    const urlSkipList =
      _parameters$urlSkipLi === void 0 ? [] : _parameters$urlSkipLi;
    const webReferrer = parameters.webReferrer;
    ...
    const aggregateValues = aggregateValuesFromParameters(
      parameters.afParameters
    );
    const currentURLParams = getCurrentURLParams(aggregateValues);
    const validParams = validateAndMappedParams(
      parameters.afParameters,
      currentURLParams
    );
    if (!validParams) return null;
    const afParams = _objectSpread2(
      {
        af_js_web: true,
    ...
    const finalParams = stringifyParameters(afParams).replace('&', '?');
    let finalURL = oneLinkURL + finalParams;
    const webReferrerValue = JSON.parse(
      localStorage.getItem(LOCAL_STORAGE_VALUES.SS_WEB_REFERRER)
    );
    if (webReferrer && webReferrerValue && isValidUrl(webReferrerValue)) {
      const webReferrerKeyValue = '&'
        .concat(webReferrer, '=')
    ...
    window.AF_SMART_SCRIPT.displayQrCode = function (htmlId) {
      const qrOptions =
        arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
      const _qrOptions$logo = qrOptions.logo;
      const logo = _qrOptions$logo === void 0 ? '' : _qrOptions$logo;
      const _qrOptions$codeColor = qrOptions.codeColor;
      const codeColor =
      ...
      // Use window.QRCode instead of QRCode (which is a local variable inside QRCode() IIFE)
      if (!window.QRCode) {
        console.error('[OneLink] QRCode not available on window object');
        return null;
      }
  ...
  const generateDirectClickURL = function generateDirectClickURL() {
    let _campaign$keys;
    const parameters =
      arguments.length > 0 && arguments[0] !== undefined
        ? arguments[0]
    ...
    const _parameters$afParamet3 = parameters.afParameters;
    const afParameters =
      _parameters$afParamet3 === void 0 ? {} : _parameters$afParamet3;
    const _parameters$referrerS2 = parameters.referrerSkipList;
    const referrerSkipList =
      _parameters$referrerS2 === void 0 ? [] : _parameters$referrerS2;
    const _parameters$urlSkipLi2 = parameters.urlSkipList;
    const urlSkipList =
      _parameters$urlSkipLi2 === void 0 ? [] : _parameters$urlSkipLi2;
    const platform = parameters.platform;
    const app_id = parameters.app_id;
    const redirectURL = parameters.redirectURL;
    const mediaSource = afParameters.mediaSource;
    const campaign = afParameters.campaign;
    if (!mediaSource) {
